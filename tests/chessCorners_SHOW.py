...
def mark_corners(image: cv2.typing.MatLike, segmentation: np.ndarray) -> list[tuple[int, int]]:
    """
    ...
    lines = cv2.HoughLines(gray, 1, np.pi / 180, 20, None, 0, 0)  # Magic Number 50
    line_points = []
...
def find_chessboard_corners(image: cv2.typing.MatLike, corner_results_data: Any) -> tuple[cv2.typing.MatLike, Optional[np.ndarray]]:
    """
    ...
    for result in corner_results_data:
        ...
        if result_masks is not None:
            for mask in result_masks.data:
                corners = mark_corners(image, mask.cpu().numpy())
